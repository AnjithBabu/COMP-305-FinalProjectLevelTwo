...
public class EnemyVelocityRangeLevelThree
{
    //variables
    public float min;
    public float max;
    ...
    //Constructor
    public EnemyVelocityRangeLevelThree(float min, float max)
    {
        this.min = min;
        this.max = max;
    }
...
public class EnemyControllerLvlThree : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (this.transform.position.x > 1413f)
        {

            if (absVelX < this.enemyVelocityRange.max)
            {
                forceX = -this.enemyMoveForce;
            }
            this.moveRightSide = true;
        }
        else if (this.transform.position.x < 791f)
        {
            if (absVelX < this.enemyVelocityRange.max)
            {
                forceX = this.enemyMoveForce;
